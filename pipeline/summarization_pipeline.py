...
Fixed: proper JSON parsing, fallback handling
"""
...
class SummarizationPipeline:
    def __init__(self, config):
        ...
        logger.info("SummarizationPipeline ready")

    def summarize(self, text: str, style: str = "detailed") -> dict:
        if not text or len(text.strip()) < 20:
            return {"error": "Text too short to summarize (min 20 chars)"}
        wc = len(text.split())
        if style == "brief":
            return self._brief(text, wc)
        if style == "bullets":
            return self._bullets(text, wc)
        return self._detailed(text, wc)

    def _detailed(self, text: str, wc: int) -> dict:
        system = (
            "Analyze this transcript and return ONLY valid JSON (no markdown, no code blocks):\n"
        ...
        try:
            ...
            # Fallback: return as plain overview
            return {
                "overview":      raw,
                "key_points":    [],
                "topics":        [],
    ...
    def _brief(self, text: str, wc: int) -> dict:
        raw = self._call(
            "Summarize in 2-3 clear sentences. Be concise.",
            text, 256
        )
    ...
    def _bullets(self, text: str, wc: int) -> dict:
        raw  = self._call(
            "List the key points as bullet points. Format each line as: • point",
            text, 512
        )
