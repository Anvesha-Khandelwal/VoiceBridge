...
class User(UserMixin, db.Model):
    ...
    def to_dict(self): return {'id':self.id,'email':self.email,'username':self.username,'theme':self.theme,'created_at':self.created_at.isoformat()}
...
class Translation(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    def to_dict(self): return {'id':self.id,'original_text':self.original_text,'translated_text':self.translated_text,'source_lang':self.source_lang,'target_lang':self.target_lang,'created_at':self.created_at.strftime('%d %b %Y, %I:%M %p')}
...
class RAGSession(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    def to_dict(self): return {'id':self.id,'session_id':self.session_id,'title':self.title,'created_at':self.created_at.strftime('%d %b %Y, %I:%M %p'),'word_count':len(self.transcript.split())}
